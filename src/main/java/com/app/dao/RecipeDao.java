...
public interface RecipeDao extends JpaRepository<Recipe,Integer> {
    ...
    @Query(nativeQuery = true, value = "select r.* from recipe as r, user_collects as uc where r.reid = uc.reid and uc.uid = ?1 and uc.type = 0 order by uc.time desc")
    ...
    @Query(nativeQuery = true,value = "select re.* from recommend as r , recipe as re, user_info as u where r.reid=re.reid and r.uid=u.uid and r.uid=?1 order by derive_time desc")
    List<Recipe> findRecommendRecipeByUid(Integer uid);
